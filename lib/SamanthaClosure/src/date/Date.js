...
goog.provide('SamanthaClosure.date');
goog.provide('SamanthaClosure.date.Date');
 ...
 */
SamanthaClosure.date.Date = function() {
    goog.base(this);
};
goog.inherits(SamanthaClosure.date.Date, goog.date.Date);
 ...
 */
SamanthaClosure.date.randomTime = function() {
 ...
 * @return {Date} random time.
 */
SamanthaClosure.date.randomTimeInBetween = function(Date1, Date2) {
 ...
 * @return {Date} Random time.
 */
SamanthaClosure.date.randomTimeInInterval = function(interval) {
    var now = new goog.date.DateTime(),
        then = now.clone();
    then.add(interval);

    return SamanthaClosure.date.randomTimeInBetween(now, then);
};
...
(function() {
     ...
     *
     * Usage: SamanthaClosure.date.formatMilliseconds(1320676229977, 'd MMMM, EEEE') returns "7 Kasım, Pazartesi".
     ...
     */
    SamanthaClosure.date.formatMilliseconds = function(milliseconds, pattern, opt_timeZone) {
 ...
 * @return {Date} Day.
 */
SamanthaClosure.date.getDay = function(date) {
    var timestamp = date.getTime();
    return new Date(SamanthaClosure.date.getDayTimestamp(timestamp));
};
 ...
 * @return {number} Day.
 */
SamanthaClosure.date.getDayTimestamp = function(timestamp) {
